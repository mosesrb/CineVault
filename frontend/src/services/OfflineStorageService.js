...
export const OfflineStorageService = {
  ...
  async downloadMedia(mediaId, url, type, metadata, token) {
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      progressListener = await Filesystem.addListener('progress', handler);
      // Optional: secondary listener for fallback
      this.secondaryListener = await Filesystem.addListener('downloadFileProgress', handler);
    } else {
    ...
    try {
      const fileName = `${type}_${mediaId}.mp4`;
      ...
      // 1. Update DB FIRST to ensure status is 'completed'
      await db.put(DOWNLOADS_STORE, {
        id: mediaId,
        status: 'completed',
        type,
      ...
      // 2. Then emit final 100% UI signal
      this._emitProgress(mediaId, 100);
      return fileName;
    } catch (err) {
