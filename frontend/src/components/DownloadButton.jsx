...
export default function DownloadButton({ mediaId, url, type, metadata, variant = 'default' }) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      setStatus('checking');
      try {
        // Proactive check: is this item currently being downloaded in the background?
        if (OfflineStorageService.isDownloading(mediaId)) {
          setStatus('downloading');
          // Subscribe will handle the progress updates
    ...
    window.addEventListener('cv_download_change', handleGlobalChange);
    init();

    return () => {
      if (unsubscribe) unsubscribe();
      window.removeEventListener('cv_download_change', handleGlobalChange);
    };
  }, [mediaId]);
          ...
          <div className="download-active-content">
            ...
            {isDetail && (
              <div className="download-text-group">
                <span className="text-accent font-bold">Saving {downloadProgress > 0 ? `${downloadProgress}%` : '...'}</span>
              </div>
            )}
          </div>
        </div>
